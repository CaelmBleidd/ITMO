...
long long amount_partition(long long n, long long k) {
	if (n == 0 && k == 0)
		return 0;
	else if (k == 0 && n != 0)
		return 1;
	else if (k > n)
		return amount_partition(n, n);
	else
		return amount_partition(n, k - 1) + amount_partition(n - k, k);
}
...
int main() {
	...
	//	long long amount = amount_partition(n,n);
