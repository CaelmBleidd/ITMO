...
long long sos_please(vector<vector<long long>> d,  long long n, long long k) {
	if (n < k) {
		return 0;
	}
	if (d[n][k] != -1) {
	...
	d[n][k] = sos_please(d, n - k, k) + sos_please(d, n, k + 1);
	return d[n][k];
}

long long part2num(vector<long long> v) {
	vector<vector<long long> > d;
	for (int i = 0; i <= v.size(); i++) {
		vector<long long> e(v.size() + 1, LONG_MIN);
		d.push_back(e);
	}

	long long ans = 0; 
	long long tmp = v.size();

	for (long long i : v) {
		ans += sos_please(d,tmp, i + 1);
		tmp -= i;
	}
	ans = sos_please(d,v.size(), 1) - ans - 1;
	return ans;
}
