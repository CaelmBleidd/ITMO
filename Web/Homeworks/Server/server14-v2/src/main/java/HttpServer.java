...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        if (file.isFile()) {
            ...
            response.setHeader("ETag", Long.toString((long) Arrays.hashCode(body)));
            if (request.headers.containsKey("If-None-Match") && request.getHeader("If-None-Match").equals(response.headers.get("ETag"))) {
                response.setStatusCode(304);
                response.setBody("".getBytes());
                response.setHeader("Content-Length", Integer.toString(0));
            }
        } else {
